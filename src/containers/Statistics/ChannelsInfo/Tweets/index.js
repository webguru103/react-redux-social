    ...
    getTotaldata() {
        ...
        switch(this.state.channel.connection.channel){
            case 'pinterest':
                ...
                last = keys[0];
                if( last != null ) {
                    total = totalInfo[last].pin_count != null ? totalInfo[last].pin_count : '0';
                    tweet = totalInfo[last].repins + " Repins";
                    contents = total + " Pins";
                    content = "Pins";
                }
                break;
            case 'twitter':
                ...
                last = keys[0];
                if( last != null ) {
                    total = totalInfo[last].tweet_count != null ? totalInfo[last].tweet_count : '0';
                    tweet = totalInfo[last].retweet_count + " Retweets";
    ...
    getChartsData() {
        ...
        switch(this.state.channel.connection.channel) {
            case 'pinterest':
                ...
                for(i in pinterest) {
                    temp = { name: this.getNameDate(pinterest[i].top_pins_by_engagement["0"].created_at), followers: pinterest[i].repins, amt: pinterest[i].repins };
                    data.push(temp);
                }
                break;
            case 'twitter':
                ...
                for(i in twitter) {
                    temp = { name: this.getNameDate(twitter[i].top_tweets_by_engagement["0"].created_at), followers: twitter[i].retweet_count, amt: twitter[i].retweet_count };
                    data.push(temp);
                }
                break;
            case 'facebook':
                ...
                for(i in facebook) {
                    temp = { name: this.getNameDate(facebook[i].top_posts_by_engagement["0"].created_time), followers: facebook[i].post_count, amt: facebook[i].post_count };
                    data.push(temp);
                }
                break;
            case 'linkedin':
                ...
                for(i in linkedin) {
                    temp = { name: this.getNameDate(linkedin[i].top_posts_by_engagement["0"].timestamp), followers: linkedin[i].post_count, amt: linkedin[i].post_count };
                    data.push(temp);
                }
                break;
            default:
